...
from app.mailer import send_verification_code

router = APIRouter(prefix="/api/auth", tags=["auth"])


def _issue_verification_code(db: Session, user: models.User) -> str:
    code = generate_verification_code()
    db.add(models.VerificationCode(
        user_id=user.id, code=code, expires_at=verification_code_expiry(),
    ))
    db.commit()
    return code
